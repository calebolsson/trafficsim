...
int getFrontPriority( PriorityQueue *pq )
{
    if( isEmptyPQ( pq ) )
    {
        ...
        fprintf( stderr, "ERROR - getFrontPriority: attempt to get priority of empty priority queue.\n" );
        return -1;
    }
...
void enqueueByPriority( PriorityQueue *pq, priorityQueueType qt, int priority )
{
    ...
    if(node==NULL)
    {
        fprintf( stderr, "ERROR - enqueueByPriority: Failed to allocate memory\n");
        exit(-1);
    }
...
bool isEmptyPQ( PriorityQueue *pq )
{
    ...
    {
        fprintf( stderr, "ERROR - isEmptyPQ: PriorityQueue had inconsistent values for front and rear.\n" );
        exit(-1);
    }
    return false;
}
